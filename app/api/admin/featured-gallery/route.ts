...
import { getFeaturedGallery, saveFeaturedGallery } from '@/lib/content';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const key = `featured-gallery/${id}.${extFor(file.type)}`;
    ...
    const item: GalleryItem = {
      ...
      alt: (form.get('alt') as string) || 'Kayak Adventure featured preview',
    ...
    const list = await getFeaturedGallery();
    list.unshift(item);
    await saveFeaturedGallery(list);
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (item) {
      ...
      await saveFeaturedGallery(list.filter((i) => i.id !== id));
    }
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    const currentList = await getFeaturedGallery();
    ...
    await saveFeaturedGallery(reordered);
